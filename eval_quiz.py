...
def _question_generation_provider(question: Dict[str, Any]) -> str:
    provider = str(question.get("generation_provider", "")).strip().lower()
    if provider:
        return provider

    if isinstance(question.get("metadata"), dict):
        provider = str(question["metadata"].get("generation_provider", "")).strip().lower()
        if provider:
            return provider
...
def evaluate_quiz_bank(
    ...
    fallback_client: Optional[CodexLLMClient] = None,
    ...
    primary_eval_provider = str(getattr(client, "primary_provider", getattr(client, "provider", ""))).strip().lower()
    fallback_eval_provider = ""
    if fallback_client is not None:
        fallback_eval_provider = str(getattr(fallback_client, "primary_provider", getattr(fallback_client, "provider", ""))).strip().lower()
    ...
    for idx, q in enumerate(quiz_bank.get("questions", []), start=1):
        ...
        generation_provider = _question_generation_provider(q)

        chosen_client = client
        chosen_eval_provider = primary_eval_provider

        if generation_provider and chosen_eval_provider and generation_provider == chosen_eval_provider:
            if fallback_client is not None and fallback_eval_provider and fallback_eval_provider != generation_provider:
                chosen_client = fallback_client
                chosen_eval_provider = fallback_eval_provider
            else:
                eval_result = {
                    "groundedness": 1,
                    "clarity": 1,
                    ...
                    "notes": (
                        ...
                        f"primary evaluator '{primary_eval_provider}', fallback evaluator '{fallback_eval_provider or 'none'}'."
                    )
                }
                ...
                results.append(row)
                print(f"[EVAL {idx}/{len(quiz_bank.get('questions', []))}] question_id={q.get('question_id', '')} (self-provider guard)")
                continue
        ...
        try:
            ...
            if (
                ...
                and chosen_client is client
                and fallback_eval_provider
                and fallback_eval_provider != generation_provider
            ):
                try:
                    raw = fallback_client.generate(system_prompt, user_prompt)
                    eval_result = _safe_parse_eval(raw)
                    chosen_eval_provider = fallback_eval_provider
                    ...
                    eval_result = {
                        ...
                        "hallucinated": True,
                        "decision": "drop",
                        "notes": f"Evaluation request failed on primary and fallback: {e} | {fallback_error}"
                    }
