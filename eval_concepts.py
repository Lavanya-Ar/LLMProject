...
def evaluate_concepts(
    ...
    api_key: str,
    eval_provider: str,
    eval_fallback_provider: str = "",
) -> Dict[str, Any]:
    primary_client = CodexLLMClient(
        model=model,
    ...
    if eval_fallback_provider:
        fallback_client = CodexLLMClient(
            model=_model_for_provider(eval_fallback_provider),
    ...
    concepts_by_segment: Dict[int, List[Dict[str, str]]] = {}
    generation_provider_by_segment: Dict[int, str] = {}
    print("DEBUG concept entries loaded:", len(concepts))
    for entry in concepts:
        print("DEBUG segment entry:", entry.get("segment_id"), "concept count:", len(entry.get("concepts", [])))
        seg_id = entry["segment_id"]
        generation_provider_by_segment[seg_id] = str(entry.get("generation_provider", "")).strip().lower()
        items = []
        for concept in entry.get("concepts", []):
            items.append({
                "concept_id": concept.get("concept_id", ""),
...
""".strip()
    ...
    for seg in segments:
        ...
        generation_provider = generation_provider_by_segment.get(seg_id, "")
        chosen_client = primary_client
        chosen_eval_provider = eval_provider

        if generation_provider and generation_provider == eval_provider:
            if fallback_client is not None and eval_fallback_provider and eval_fallback_provider != generation_provider:
                chosen_client = fallback_client
                chosen_eval_provider = eval_fallback_provider
            else:
                results.append({
                    "segment_id": seg_id,
                    ...
                    "skipped": True,
                    "notes": (
                        f"Skipped evaluation to avoid self-provider judging: generated by '{generation_provider}', "
                        f"primary evaluator '{eval_provider}', fallback evaluator '{eval_fallback_provider or 'none'}'."
                    ),
                    "evaluations": [],
                })
                continue
        ...
        try:
            raw = chosen_client.generate(system_prompt, user_prompt)
            print(f"DEBUG eval raw for segment {seg_id}: {raw}")
        except Exception as e:
            print(f"Skipping segment {seg_id} due to API error: {e}")
            continue
        ...
        results.append({
            "segment_id": seg_id,
            "generation_provider": generation_provider,
            "evaluation_provider": chosen_eval_provider,
            "skipped": False,
            "evaluations": eval_items,
        })
...
def main() -> None:
    ...
    if eval_provider == "openrouter":
        default_model = os.getenv("OPENROUTER_EVAL_MODEL", os.getenv("OPENROUTER_MODEL", "openrouter/free"))
    elif eval_provider == "mistral":
        default_model = os.getenv("MISTRAL_EVAL_MODEL", os.getenv("MISTRAL_MODEL", "mistral-small-latest"))
    elif eval_provider == "nim":
        default_model = os.getenv("NIM_EVAL_MODEL", os.getenv("NIM_MODEL", "meta/llama-3.1-8b-instruct"))
    elif eval_provider == "groq":
        default_model = os.getenv("GROQ_EVAL_MODEL", os.getenv("GROQ_MODEL", "llama-3.1-8b-instant"))
    else:
        raise ValueError(f"Unsupported EVAL_LLM_PROVIDER: {eval_provider}")

    parser.add_argument("--eval-provider", default=eval_provider)
    parser.add_argument("--eval-fallback-provider", default=os.getenv("EVAL_FALLBACK_LLM_PROVIDER", ""))
    parser.add_argument("--model", default=default_model)
    ...
    report = evaluate_concepts(
        ...
        eval_fallback_provider=args.eval_fallback_provider,
    )
